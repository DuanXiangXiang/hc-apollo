...
/** entrance */
//应用负责人--下拉列表数据展示
directive_module.directive('apollouserselector', function ($compile, $window,AppUtil,UserService,PermissionService) {
    return {
        restrict: 'E',
        ...
        link: function (scope, element, attrs) {
            ...
            //获取当前用户是否拥有权限，true--管理员  false--普通用户
            PermissionService.has_root_permission().then(function (result) {
                scope.hasRootPermission = result.hasPermission;
            })
            var select2Options = {
                ajax: {
                    ...
                    processResults: function (data, params) {
                        var users = [];
                        //增加一个判断条件 如果是管理员则将所有用户传送到前端页面，反之，将只传当前用户
                        if(scope.hasRootPermission){
                            ...
                            users.push({
                                id: scope.userName,
                                text: scope.userName + " | " + scope.userName
                            })
                        }
                        return {
                            results: users
...
//项目管理员--下拉列表数据展示
directive_module.directive('apollomultipleuserselector', function ($compile, $window,AppUtil,UserService,PermissionService) {
    return {
        restrict: 'E',
